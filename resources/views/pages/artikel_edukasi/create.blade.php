...
.gb-panel-tab button.active {
    color: #EC1E88;
}
.gb-panel-tab button.active::after {
    content: '';
    position: absolute;
    bottom: -2px;
    left: 0; right: 0;
...
.gb-featured-img .upload-hint {
    font-size: 0.78rem;
    color: #94a3b8;
    text-align: center;
    z-index: 1;
}
.gb-featured-img .remove-img-btn {
    position: absolute;
    top: 6px;
    right: 6px;
...
<form id="artikel-form" action="{{ route('artikel-edukasi.store') }}" method="POST" enctype="multipart/form-data">
@csrf
...
<div class="gb-toolbar">
    ...
    <span class="article-title-bar" id="title-bar-preview">Artikel baru…</span>
    <span id="toolbar-status-badge" class="status-indicator draft">
        <i class="bi bi-circle-fill" style="font-size:.5rem;"></i> Draft
    </span>
    ...
    <div class="gb-editor-card">
        <div class="gb-editor-inner">
            ...
            <textarea id="judul-input" name="judul" class="gb-title-input @error('judul') is-invalid @enderror"
                      rows="2" placeholder="Tulis judul artikel di sini…" required>{{ old('judul') }}</textarea>
            @error('judul')
            ...
            <div class="gb-slug-row">
                ...
                <span class="gb-slug-val" id="slug-preview">/artikel/<em id="slug-val">judul-artikel</em></span>
            </div>
            ...
            <div>
                <textarea id="tinymce-editor" name="isi"
                          ...
                          style="visibility:hidden;">{{ old('isi') }}</textarea>
                @error('isi')
        ...
        <div id="tab-post" class="gb-panel-body">
            ...
            <div class="gb-panel-section">
                ...
                <button type="button" class="btn-publish" onclick="submitAs('published')">
                    <i class="bi bi-globe"></i> Terbitkan Sekarang
                </button>
            ...
            <div class="gb-panel-section">
                ...
                <input type="date" name="diterbitkan_pada" id="tgl-terbit"
                    class="form-control form-control-sm @error('diterbitkan_pada') is-invalid @enderror"
                    value="{{ old('diterbitkan_pada', date('Y-m-d')) }}"
            ...
            <div class="gb-panel-section">
                ...
                <select name="kategori_artikel_id" class="form-select form-select-sm @error('kategori_artikel_id') is-invalid @enderror"
                    style="border-radius:8px;" required>
                    <option value="">— Pilih Kategori —</option>
                    @foreach($kategoris as $kat)
                        <option value="{{ $kat->id }}" {{ old('kategori_artikel_id') == $kat->id ? 'selected' : '' }}>
                            {{ $kat->nama }}
                        </option>
                    @endforeach
                </select>
                @error('kategori_artikel_id')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
                <a href="{{ route('kategori-artikel.create') }}" target="_blank"
                   class="d-block mt-2 text-decoration-none" style="font-size:0.76rem;color:#EC1E88;">
            ...
            <div class="gb-panel-section">
                ...
                <input type="text" name="penulis" class="form-control form-control-sm @error('penulis') is-invalid @enderror"
                    value="{{ old('penulis', auth()->user()->name ?? '') }}"
                    placeholder="Nama penulis artikel" style="border-radius:8px;" required>
                @error('penulis')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
        ...
        <div id="tab-block" class="gb-panel-body" style="display:none;">
            <div class="gb-panel-section">
                ...
                <div class="gb-featured-img" id="feat-img-zone">
                    ...
                    <button type="button" class="remove-img-btn" id="feat-img-remove" onclick="removeFeaturedImg()">
                ...
                <input type="file" id="gambar-input" name="gambar" class="d-none"
                       ...
                       @error('gambar') aria-describedby="gambar-error" @enderror>
                @error('gambar')
                    <div class="text-danger mt-1" id="gambar-error" style="font-size:.8rem;">{{ $message }}</div>
                @enderror
                <button type="button" class="btn btn-sm btn-outline-secondary w-100 mt-2"
                        style="border-radius:8px;font-size:.78rem;" onclick="document.getElementById('gambar-input').click()">
                    <i class="bi bi-folder2-open me-1"></i> Pilih Gambar
                </button>
            </div>
        </div>
    </div>
...
tinymce.init({
    ...
    setup: function(editor) {
        ...
        editor.on('init', function() {
            editor.setContent({!! json_encode(old('isi')) !!});
        });
        @endif
    }
});
...
judulInput.addEventListener('input', function () {
    ...
    slugVal.textContent = val ? generateSlug(val) : 'judul-artikel';
    titleBarPrev.textContent = val || 'Artikel baru…';
});
...
featZone.addEventListener('click', function(e) {
    if (e.target === featRemove || featRemove.contains(e.target)) return;
    gambarInput.click();
});
...
featZone.addEventListener('dragover', e => { e.preventDefault(); featZone.style.borderColor = '#EC1E88'; });
featZone.addEventListener('dragleave', () => { featZone.style.borderColor = '#e2e8f0'; });
featZone.addEventListener('drop', e => {
    e.preventDefault();
    featZone.style.borderColor = '#e2e8f0';
