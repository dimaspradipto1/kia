            ...
            <div class="col-lg-12">
                <div class="card border-0 shadow-sm mb-4" style="border-radius:12px; border-top:5px solid #EC1E88 !important;">
                    <div class="card-header bg-white border-bottom py-3 px-4">
                        <h5 class="m-0 fw-bold text-dark">Form Edit KB Pasca Salin</h5>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('kb-pasca-salin.update', $kbPascaSalin->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                                ...
                                <div class="col-md-12 mb-3">
                                    ...
                                    <select name="buku_kia_id" class="form-select select2 @error('buku_kia_id') is-invalid @enderror" data-placeholder="Pilih Pasien / Buku KIA" required>
                                        ...
                                        @foreach ($bukuKias as $bk)
                                            <option value="{{ $bk->id }}" {{ old('buku_kia_id', $kbPascaSalin->buku_kia_id) == $bk->id ? 'selected' : '' }}>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="nakes_id" class="form-select select2 @error('nakes_id') is-invalid @enderror" data-placeholder="Pilih Tenaga Kesehatan" required>
                                        <option value=""></option>
                                        @foreach ($nakes as $n)
                                            <option value="{{ $n->id }}" {{ old('nakes_id', $kbPascaSalin->nakes_id) == $n->id ? 'selected' : '' }}>
                                                {{ $n->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="fasilitas_kesehatan_id" class="form-select select2 @error('fasilitas_kesehatan_id') is-invalid @enderror" data-placeholder="Pilih Fasilitas Kesehatan" required>
                                        <option value=""></option>
                                        @foreach ($faskes as $f)
                                            <option value="{{ $f->id }}" {{ old('fasilitas_kesehatan_id', $kbPascaSalin->fasilitas_kesehatan_id) == $f->id ? 'selected' : '' }}>
                                                {{ $f->nama_faskes }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="metode_kb" class="form-select @error('metode_kb') is-invalid @enderror" required>
                                        ...
                                        @foreach (['MAL (Metode Amenore Laktasi)', 'Kondom', 'Pil KB', 'Suntik KB', 'Implan / Susuk', 'IUD / Spiral', 'MOW (Steril Wanita)', 'MOP (Steril Pria)', 'Lainnya'] as $m)
                                            <option value="{{ $m }}" {{ old('metode_kb', $kbPascaSalin->metode_kb) == $m ? 'selected' : '' }}>{{ $m }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="tanggal_mulai" class="form-control datepicker @error('tanggal_mulai') is-invalid @enderror" value="{{ old('tanggal_mulai', $kbPascaSalin->tanggal_mulai) }}" placeholder="Pilih Tanggal" required>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <textarea name="catatan" class="form-control @error('catatan') is-invalid @enderror" rows="4" placeholder="Keterangan tambahan atau catatan medis KB...">{{ old('catatan', $kbPascaSalin->catatan) }}</textarea>
