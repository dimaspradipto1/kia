...
@section('content')
    ...
    <section class="section animate__animated animate__fadeIn">
        <form action="{{ route('pemantauan-nifas.update', $pemantauanNifas->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-lg-5 mb-4">
                    <div class="card border-0 shadow-sm mb-4" style="border-radius:12px; border-top:5px solid #EC1E88 !important; height: 100%;">
                        <div class="card-header bg-white border-bottom py-3 px-4">
                            <h5 class="m-0 fw-bold text-dark"><i class="bi bi-pencil-square text-pink me-1"></i> Edit Kunjungan Nifas</h5>
                        </div>
                        <div class="card-body p-4">
                            <div class="mb-3">
                                ...
                                <select name="buku_kia_id" class="form-select select2 @error('buku_kia_id') is-invalid @enderror" data-placeholder="Pilih Pasien / Buku KIA" required>
                                    ...
                                    @foreach ($bukuKias as $bk)
                                        <option value="{{ $bk->id }}" {{ old('buku_kia_id', $pemantauanNifas->buku_kia_id) == $bk->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="nakes_id" class="form-select select2 @error('nakes_id') is-invalid @enderror" data-placeholder="Pilih Tenaga Kesehatan" required>
                                    <option value=""></option>
                                    @foreach ($nakes as $n)
                                        <option value="{{ $n->id }}" {{ old('nakes_id', $pemantauanNifas->nakes_id) == $n->id ? 'selected' : '' }}>
                                            {{ $n->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="tanggal" class="form-control datepicker @error('tanggal') is-invalid @enderror" value="{{ old('tanggal', $pemantauanNifas->tanggal) }}" placeholder="Pilih Tanggal" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="hari_ke" class="form-select @error('hari_ke') is-invalid @enderror" required>
                                        ...
                                        @foreach (['KF 1 (6 Jam - 3 Hari)', 'KF 2 (4 Hari - 28 Hari)', 'KF 3 (29 Hari - 42 Hari)'] as $kf)
                                            <option value="{{ $kf }}" {{ old('hari_ke', $pemantauanNifas->hari_ke) == $kf ? 'selected' : '' }}>{{ $kf }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="catatan" class="form-control @error('catatan') is-invalid @enderror" rows="5" placeholder="Keterangan tambahan atau rekomendasi bidan/dokter...">{{ old('catatan', $pemantauanNifas->catatan) }}</textarea>
                                        ...
                                        <div class="d-flex justify-content-between align-items-center flex-wrap">
                                            ...
                                            <div class="d-flex gap-3 mt-2 mt-sm-0">
                                                <div class="form-check form-check-inline m-0">
                                                    <input class="form-check-input" type="radio" name="{{ $field }}" id="{{ $field }}_ya" value="Ya" {{ old($field, $pemantauanNifas->$field) == 'Ya' ? 'checked' : '' }} required>
                                                    <label class="form-check-label fw-bold text-danger small cursor-pointer" for="{{ $field }}_ya">Ya</label>
                                                </div>
                                                <div class="form-check form-check-inline m-0">
                                                    <input class="form-check-input" type="radio" name="{{ $field }}" id="{{ $field }}_tidak" value="Tidak" {{ old($field, $pemantauanNifas->$field) == 'Tidak' ? 'checked' : '' }} required>
