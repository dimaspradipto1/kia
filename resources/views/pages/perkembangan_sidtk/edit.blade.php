...
@section('content')
    ...
    <section class="section animate__animated animate__fadeIn">
        <form action="{{ route('perkembangan-sidtk.update', $perkembanganSidtk->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                {{-- Kolom Kiri --}}
                    ...
                    <div class="card border-0 shadow-sm h-100" style="border-radius:12px; border-top:5px solid #7c3aed !important;">
                        <div class="card-header bg-white border-bottom py-3 px-4">
                            <h5 class="m-0 fw-bold text-dark">
                                <i class="bi bi-pencil-square me-1" style="color:#7c3aed;"></i> Edit Identitas & Petugas
                            </h5>
                        </div>
                        <div class="card-body p-4">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="profil_anak_id" class="form-select select2 @error('profil_anak_id') is-invalid @enderror"
                                    ...
                                    @foreach ($profilAnaks as $pa)
                                        <option value="{{ $pa->id }}" {{ old('profil_anak_id', $perkembanganSidtk->profil_anak_id) == $pa->id ? 'selected' : '' }}>
                                            {{ $pa->nama_lengkap }}
                            ...
                            <div class="mb-3">
                                ...
                                <select name="nakes_id" class="form-select select2 @error('nakes_id') is-invalid @enderror"
                                    ...
                                    <option value=""></option>
                                    @foreach ($nakes as $n)
                                        <option value="{{ $n->id }}" {{ old('nakes_id', $perkembanganSidtk->nakes_id) == $n->id ? 'selected' : '' }}>
                                            {{ $n->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="tanggal_skrining" class="form-control datepicker @error('tanggal_skrining') is-invalid @enderror"
                                       value="{{ old('tanggal_skrining', $perkembanganSidtk->tanggal_skrining->format('Y-m-d')) }}" required>
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group">
                                    <input type="number" name="usia_bulan" class="form-control @error('usia_bulan') is-invalid @enderror"
                                           value="{{ old('usia_bulan', $perkembanganSidtk->usia_bulan) }}" min="0" max="72" required>
                ...
                <div class="col-lg-7 mb-4">
                                    ...
                                    @foreach ($domains as $domain)
                                        ...
                                        <div class="col-12 col-sm-6">
                                            <input type="radio" class="btn-check" name="domain" id="domain_{{ $dc['id'] }}"
                                                   value="{{ $domain }}"
                                                   {{ old('domain', $perkembanganSidtk->domain) == $domain ? 'checked' : '' }} required>
                                    ...
                                    @foreach ($hasilOptions as $h)
                                        ...
                                        <div class="flex-grow-1">
                                            <input type="radio" class="btn-check" name="hasil" id="hasil_{{ Str::slug($h) }}"
                                                   value="{{ $h }}"
                                                   {{ old('hasil', $perkembanganSidtk->hasil) == $h ? 'checked' : '' }} required>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="tindak_lanjut" class="form-select @error('tindak_lanjut') is-invalid @enderror" required>
                                    ...
                                    @foreach ($tindakLanjuts as $tl)
                                        <option value="{{ $tl }}" {{ old('tindak_lanjut', $perkembanganSidtk->tindak_lanjut) == $tl ? 'selected' : '' }}>
                                            {{ $tl }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="text-end">
                        ...
                        <button type="submit" class="btn text-white px-5 shadow-sm"
                            ...
                            <i class="bi bi-save me-1"></i> Perbarui Skrining
                        </button>
                    </div>
                </div>
            </div>
