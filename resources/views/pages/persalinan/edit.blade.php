...
@section('content')
    ...
    <section class="section animate__animated animate__fadeIn">
        <form action="{{ route('persalinan.update', $persalinan->id) }}" method="POST">
            @csrf
            @method('PUT')
                ...
                <div class="col-lg-6 mb-4">
                    <div class="card border-0 shadow-sm" style="border-radius:12px; border-top:5px solid #7E22CE !important; height: 100%;">
                        <div class="card-header bg-white border-bottom py-3 px-4">
                            <h5 class="m-0 fw-bold text-dark"><i class="bi bi-pencil-square text-purple me-1"></i> Edit Informasi Persalinan</h5>
                        </div>
                        <div class="card-body p-4">
                            <div class="mb-3">
                                ...
                                <select name="buku_kia_id" class="form-select select2 @error('buku_kia_id') is-invalid @enderror" data-placeholder="Pilih Pasien / Buku KIA" required>
                                    ...
                                    @foreach ($bukuKias as $bk)
                                        <option value="{{ $bk->id }}" {{ old('buku_kia_id', $persalinan->buku_kia_id) == $bk->id ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="fasilitas_kesehatan_id" class="form-select select2 @error('fasilitas_kesehatan_id') is-invalid @enderror" data-placeholder="Pilih Tempat Persalinan (Faskes)" required>
                                    <option value=""></option>
                                    @foreach ($faskes as $f)
                                        <option value="{{ $f->id }}" {{ old('fasilitas_kesehatan_id', $persalinan->fasilitas_kesehatan_id) == $f->id ? 'selected' : '' }}>
                                            {{ $f->nama_faskes }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" name="tanggal_lahir" class="form-control datepicker @error('tanggal_lahir') is-invalid @enderror" value="{{ old('tanggal_lahir', $persalinan->tanggal_lahir) }}" placeholder="Pilih Tanggal" required>
                                    @error('tanggal_lahir') <div class="invalid-feedback">{{ $message }}</div> @enderror
                                </div>

                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">Jam Lahir <span class="text-danger">*</span></label>
                                    <input type="text" name="jam_lahir" class="form-control timepicker @error('jam_lahir') is-invalid @enderror" value="{{ old('jam_lahir', $persalinan->jam_lahir) }}" placeholder="Contoh: 08:30" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="jenis_persalinan" class="form-select @error('jenis_persalinan') is-invalid @enderror" required>
                                        ...
                                        @foreach (['Spontan / Normal', 'Seksio Sesarea (Caesar)', 'Vakum Ekstraksi', 'Forsep', 'Induksi'] as $type)
                                            <option value="{{ $type }}" {{ old('jenis_persalinan', $persalinan->jenis_persalinan) == $type ? 'selected' : '' }}>{{ $type }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="penolong" class="form-select @error('penolong') is-invalid @enderror" required>
                                        ...
                                        @foreach (['Dokter Kandungan (Sp.OG)', 'Dokter Umum', 'Bidan', 'Dukun Beranak', 'Lainnya'] as $p)
                                            <option value="{{ $p }}" {{ old('penolong', $persalinan->penolong) == $p ? 'selected' : '' }}>{{ $p }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="nakes_id" class="form-select select2 @error('nakes_id') is-invalid @enderror" data-placeholder="Pilih Tenaga Kesehatan" required>
                                    <option value=""></option>
                                    @foreach ($nakes as $n)
                                        <option value="{{ $n->id }}" {{ old('nakes_id', $persalinan->nakes_id) == $n->id ? 'selected' : '' }}>
                                            {{ $n->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="card-body p-4">
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" step="0.01" name="berat_bayi_kg" class="form-control @error('berat_bayi_kg') is-invalid @enderror" value="{{ old('berat_bayi_kg', $persalinan->berat_bayi_kg) }}" placeholder="Contoh: 3.10" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" step="0.1" name="panjang_bayi_cm" class="form-control @error('panjang_bayi_cm') is-invalid @enderror" value="{{ old('panjang_bayi_cm', $persalinan->panjang_bayi_cm) }}" placeholder="Contoh: 49.5" required>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" min="0" max="10" name="apgar_score_1" class="form-control @error('apgar_score_1') is-invalid @enderror" value="{{ old('apgar_score_1', $persalinan->apgar_score_1) }}" placeholder="0 - 10" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="number" min="0" max="10" name="apgar_score_5" class="form-control @error('apgar_score_5') is-invalid @enderror" value="{{ old('apgar_score_5', $persalinan->apgar_score_5) }}" placeholder="0 - 10" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="kondisi_ibu" class="form-select @error('kondisi_ibu') is-invalid @enderror" required>
                                        ...
                                        @foreach (['Sehat', 'Sakit / Lemah', 'Meninggal Dunia', 'Pendarahan Berat', 'Preeklamsia'] as $cond)
                                            <option value="{{ $cond }}" {{ old('kondisi_ibu', $persalinan->kondisi_ibu) == $cond ? 'selected' : '' }}>{{ $cond }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="kondisi_bayi" class="form-select @error('kondisi_bayi') is-invalid @enderror" required>
                                        ...
                                        @foreach (['Sehat', 'Asfiksia', 'Prematur', 'Meninggal Dunia (IUFD)', 'Cacat Bawaan'] as $cond)
                                            <option value="{{ $cond }}" {{ old('kondisi_bayi', $persalinan->kondisi_bayi) == $cond ? 'selected' : '' }}>{{ $cond }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" name="komplikasi" class="form-control @error('komplikasi') is-invalid @enderror" value="{{ old('komplikasi', $persalinan->komplikasi) }}" placeholder="Contoh: Ketuban pecah dini, plasenta previa...">
                            ...
                            <div class="row mt-4">
                                <div class="col-12 text-end">
                                    ...
                                    <button type="submit" class="btn text-white px-5 shadow-sm" style="background-color:#7E22CE; border-radius:30px; font-weight: 600;">
                                        <i class="bi bi-save me-1"></i> Perbarui Persalinan
                                    </button>
                                </div>
                            </div>
                        </div>
