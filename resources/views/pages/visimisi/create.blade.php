            ...
            <form action="{{ route('visi-misi.store') }}" method="POST">
                    ...
                    <div class="card-body p-4">
                        <div class="mb-1">
                            ...
                            <textarea name="misi_raw" rows="6"
                                      ...
                                      placeholder="Meningkatkan akses informasi kesehatan ibu dan anak di seluruh Indonesia.&#10;Menghadirkan teknologi pemantauan kesehatan digital yang mudah dan ramah pengguna.&#10;Menjadi platform yang dapat diandalkan oleh setiap keluarga."
                                      required>{{ old('misi_raw') }}</textarea>
                    ...
                    <div class="card-body p-4">
                        <div class="mb-3">
                            ...
                            <textarea name="nilai" rows="3"
                                      ...
                                      placeholder="Transparansi informasi, empati yang tulus dalam pelayanan, inovasi tanpa henti...">{{ old('nilai') }}</textarea>
                ...
                <div class="card border-0 shadow-sm mb-4" style="border-radius:12px;border-top:5px solid #8B5CF6 !important;">
                    ...
                    <div class="card-body p-4">
                        ...
                        <div id="empty-nilai-hint" class="text-center text-muted py-4" style="border:2px dashed #e2e8f0;border-radius:12px;">
                        ...
                        {{-- Template tema color reference --}}
                        <div class="mt-4 p-3" style="background:#F8FAFC;border-radius:10px;">
                            <p class="fw-bold mb-2 small">Referensi Ikon Font Awesome (isi tanpa awalan "fa "):</p>
...
function createNilaiItem(data = {}) {
    ...
    document.getElementById('empty-nilai-hint').style.display = 'none';
...
function updateIconPreview(idx) {
    ...
    const ikonVal = ikonInput.value.trim() || 'fa-star';
    preview.className = `fa ${ikonVal} icon-preview-${idx}`;
...
function bindEvents() {
    ...
    document.querySelectorAll('.btn-remove-nilai').forEach(btn => {
        btn.onclick = function () {
            ...
            if (document.querySelectorAll('.nilai-item-row').length === 0) {
...
@if(old('nilai_items'))
    @foreach(old('nilai_items') as $i => $item)
        createNilaiItem({
            judul: '{{ addslashes($item['judul'] ?? '') }}',
            deskripsi: '{{ addslashes($item['deskripsi'] ?? '') }}',
            ikon: '{{ addslashes($item['ikon'] ?? 'fa-star') }}',
            tema: '{{ $item['tema'] ?? 'blue' }}'
        });
    @endforeach
@endif
