            ...
            <div class="col-lg-12">
                <div class="card border-0 shadow-sm mb-4" style="border-radius: 12px; border-top: 5px solid #EC1E88 !important;">
                    <div class="card-header bg-white border-bottom py-3 px-4">
                        <h5 class="m-0 fw-bold text-dark">Form Tambah Wilayah Dinkes</h5>
                    </div>
                    <div class="card-body p-4">
                        <form action="{{ route('wilaya-dinkes.store') }}" method="POST">
                            @csrf
                            <div class="row mb-3">
                                <div class="col-md-6 mb-3">
                                    <label class="form-label fw-bold">Kode Dinkes <span class="text-danger">*</span></label>
                                    <input type="text" name="kode_dinkes" class="form-control @error('kode_dinkes') is-invalid @enderror" value="{{ old('kode_dinkes') }}" required>
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select name="tipe_dinkes" class="form-select @error('tipe_dinkes') is-invalid @enderror" required>
                                        ...
                                        <option value="Provinsi" {{ old('tipe_dinkes') == 'Provinsi' ? 'selected' : '' }}>Provinsi</option>
                                        <option value="Kabupaten" {{ old('tipe_dinkes') == 'Kabupaten' ? 'selected' : '' }}>Kabupaten</option>
                                        <option value="Kota" {{ old('tipe_dinkes') == 'Kota' ? 'selected' : '' }}>Kota</option>
                                        <option value="Puskesmas" {{ old('tipe_dinkes') == 'Puskesmas' ? 'selected' : '' }}>Puskesmas</option>
                                        <option value="Lainnya" {{ old('tipe_dinkes') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                                    </select>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="nama_dinkes" class="form-control @error('nama_dinkes') is-invalid @enderror" rows="3" required>{{ old('nama_dinkes') }}</textarea>
